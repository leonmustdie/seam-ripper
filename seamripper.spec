...
TOOL_SCRIPTS = [
    ...
    "verify_lzx.py", "lua_callsplit.py", "lua_batchedit.py",
]
...
BINARIES = ["luac51.exe", "unluac.jar", "luadec.exe",
            ...
            # Build for the target OS from lzxverify_src/ (see build.sh).
            "lzxverify.exe" if os.name == "nt" else "lzxverify"]

datas = [(s, ".") for s in TOOL_SCRIPTS if os.path.exists(s)]
datas += [(s, ".") for s in OPTIONAL_SCRIPTS if os.path.exists(s)]
datas += [(b, ".") for b in BINARIES if os.path.exists(b)]
...
_tool_mods = [os.path.splitext(s)[0] for s in TOOL_SCRIPTS + OPTIONAL_SCRIPTS
              + ["pip_gfx.py"] if os.path.exists(s)]
